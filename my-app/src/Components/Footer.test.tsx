...
describe('<Footer />', () => {
  ...
  test('renders three Stay Connected links with correct destinations', () => {
    ...
    const stayConnectedHeading = screen.getByRole('heading', { name: /stay connected/i });

    const container = stayConnectedHeading.closest('.link-column-full') ?? stayConnectedHeading.parentElement;
    ...
    expect(instagram).toHaveAttribute('href', expect.stringContaining('instagram.com/styledbygio'));
  ...
  test('shows the copyright message', () => {
    ...
    expect(
        screen.getByText(/©\s*2025\s+Giovanny Collazo/i)
    ).toBeInTheDocument();
  })
});
